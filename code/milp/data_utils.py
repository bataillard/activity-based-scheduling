...
def cplex_to_df(w, x, d, tt, car_avail, mode, keys, act_id, location, minutes=False):
    ...
    solution_df = pd.DataFrame(
        columns=['act_id', 'label', 'start_time', 'end_time', 'duration', 'location', 'mode', 'travel_time'])
...
def read_schedule(hh, sched_folder="MTMC_schedules/", tt_folder="MTMC_ttmatrices/", mode="driving", compute=False,
                  verbose=False):
    fname_csv = str(hh)
    ...
    try:
        ...
        sched['location'] = sched.location.apply(
            lambda x: tuple(map(float, x[1:-1].split(','))))  # need to convert locations back to tuple after import
...
def get_param_variables(param_dict):
    p_st_e_f, p_st_l_f, p_dur_s_f, p_dur_l_f, p_st_e_m, p_st_l_m, p_dur_s_m, p_dur_l_m, p_st_e_r, p_st_l_r, p_dur_s_r, p_dur_l_r = \
    param_dict['p_st_e_f'], param_dict['p_st_l_f'], param_dict['p_dur_s_f'], param_dict['p_dur_l_f'], param_dict[
        'p_st_e_m'], param_dict['p_st_l_m'], param_dict['p_dur_s_m'], param_dict['p_dur_l_m'], param_dict['p_st_e_r'], \
    param_dict['p_st_l_r'], param_dict['p_dur_s_r'], param_dict['p_dur_l_r']
    p_t, error_w, error_z, error_x, error_d = param_dict['p_t'], param_dict['error_w'], param_dict['error_z'], \
                                              param_dict['error_x'], param_dict['error_d']

    d_st_h, d_st_w, d_st_edu, d_st_s, d_st_er, d_st_b, d_st_l, d_st_es = param_dict['d_st_h'], param_dict['d_st_w'], \
                                                                         param_dict['d_st_edu'], param_dict['d_st_s'], \
                                                                         param_dict['d_st_er'], param_dict['d_st_b'], \
                                                                         param_dict['d_st_l'], param_dict['d_st_es']

    d_dur_h, d_dur_w, d_dur_edu, d_dur_s, d_dur_er, d_dur_b, d_dur_l, d_dur_es = param_dict['d_dur_h'], param_dict[
        'd_dur_w'], param_dict['d_dur_edu'], param_dict['d_dur_s'], param_dict['d_dur_er'], param_dict['d_dur_b'], \
                                                                                 param_dict['d_dur_l'], param_dict[
                                                                                     'd_dur_es']
                ...
                for l in loc:
                    ...
                    for i in loc:
                        if i == l:
                            ...
                            dic[i] = gmaps.directions(origin, destination, mode=m)[0]['legs'][0].get('duration',
                                                                                                     {}).get('value',
                                                                                                             -1) / 3600
                    dico[l] = dic
...
def check_valid(schedule):
    ...
    labels = ['home', 'work', 'education', 'shopping', 'errands_services', 'business_trip', 'leisure', 'escort',
              'travel']
